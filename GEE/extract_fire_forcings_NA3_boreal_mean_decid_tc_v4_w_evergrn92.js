...
var land = ee.Image("users/masseyr44/decid/land_extent_NA"),
    ...
    tc1992 = ee.Image("users/masseyr44/tc_1992_median_output_lzw"),
    geometry2 = 
    /* color: #d63000 */
    /* displayProperties: [
      {
    ...
    ee.Geometry.Polygon(
        [[[-141.2749441929467, 70.20802135709287],
          [-141.2749441929467, 42.18096986656817],
          [-43.716350442946684, 42.18096986656817],
          [-43.716350442946684, 70.20802135709287]]], null, false);
...
var area_img = ee.Image.pixelArea().multiply(tc_mask_max).clip(geometry2);
...
var tc_diff = tc2015.subtract(tc1992).updateMask(tc_mask).multiply(land).multiply(0.01);
var tc_diff_new = tc2015.subtract(tc2000).updateMask(tc_mask).multiply(land).multiply(0.01);
...
Map.addLayer(tc_diff_new,{min:-0.25, max:0.25, palette: 'B62809,FFC560,1178C8'},'tc_diff_new',false);

var decid_diff = ee.Image(decid2015).subtract(ee.Image(decid1992)).updateMask(tc_mask).multiply(land).toFloat().multiply(0.01);
var decid_diff_new = ee.Image(decid2015).subtract(ee.Image(decid2000)).updateMask(tc_mask).multiply(land).toFloat().multiply(0.01);
...
//var decid_udiff = ee.Image().expression('((a**2 + b**2) ** 0.5) * 0.5',{'a': decid2000u.toFloat(), 'b': decid2015u.toFloat()}).updateMask(tc_mask).multiply(land).clip(boreal_bounds).toFloat().multiply(0.01);
