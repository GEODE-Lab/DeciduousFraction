...
var land = ee.Image("users/masseyr44/decid/land_extent_NA"),
    ...
    alb_sum2000_v0 = ee.Image("users/masseyr44/albedo_products/albedo_composite_mean_2000_2002_150_240"),
    alb_fal2000_v0 = ee.Image("users/masseyr44/albedo_products/albedo_composite_mean_2000_2002_240_300"),
    alb_spr2000_v0 = ee.Image("users/masseyr44/albedo_products/albedo_composite_mean_2000_2002_60_150"),
    alb_sum2015_v0 = ee.Image("users/masseyr44/albedo_products/albedo_composite_mean_2013_2018_150_240"),
    alb_fal2015_v0 = ee.Image("users/masseyr44/albedo_products/albedo_composite_mean_2013_2018_240_300"),
    alb_spr2015_v0 = ee.Image("users/masseyr44/albedo_products/albedo_composite_mean_2013_2018_60_150"),
...
alb_sum2000 = alb_sum2000.toFloat().multiply(0.01)
alb_spr2000 = alb_spr2000.toFloat().multiply(0.01)
alb_fal2000 = alb_fal2000.toFloat().multiply(0.01)
alb_sum2015 = alb_sum2015.toFloat().multiply(0.01)
alb_spr2015 = alb_spr2015.toFloat().multiply(0.01)
alb_fal2015 = alb_fal2015.toFloat().multiply(0.01)


Map.addLayer(alb_sum2000,{},'alb_sum2000',false);
Map.addLayer(alb_spr2000,{},'alb_spr2000',false);
Map.addLayer(alb_fal2000,{},'alb_fal2000',false);
Map.addLayer(alb_sum2015,{},'alb_sum2015',false);
Map.addLayer(alb_spr2015,{},'alb_spr2015',false);
Map.addLayer(alb_fal2015,{},'alb_fal2015',false);
...
print(tc_area_boreal)

Map.addLayer(tc_mask_boreal,{},'tc_boreal',false)
...
var fall_diff = change_calc(alb_fal2000, alb_fal2015).updateMask(tc_mask).multiply(land);
var fall_udiff = uncert_calc(alb_fal2000, alb_fal2015, alb_fall2000_uncert, alb_fall2015_uncert).updateMask(tc_mask);
...
var sum_diff = change_calc(alb_sum2000, alb_sum2015).updateMask(tc_mask).multiply(land);
var sum_udiff = uncert_calc(alb_sum2000, alb_sum2015, alb_fall2000_uncert, alb_fall2015_uncert).updateMask(tc_mask);
...
var spr_diff = change_calc(alb_spr2000, alb_spr2015).updateMask(tc_mask).multiply(land);
var spr_udiff = uncert_calc(alb_spr2000, alb_spr2015, alb_spr2015_uncert, alb_spr2015_uncert).updateMask(tc_mask);
...
var pos_decid_diff = decid_diff.updateMask(decid_diff.gt(0))
...
//print(hist_pos)
var pos_chart = ui.Chart.image.histogram(pos_decid_diff, boreal_geom, 250, 40, 0.01, null, 1e13 )
//print(pos_chart)


var neg_decid_diff = decid_diff.updateMask(decid_diff.lt(0)).abs()
...
//print(hist_neg)
var neg_chart = ui.Chart.image.histogram(neg_decid_diff, boreal_geom, 250, 40, 0.01, null, 1e13 )
//print(neg_chart)

//Map.addLayer(pos_decid_diff,{min:0, max:1},'pos_decid_diff')
//Map.addLayer(neg_decid_diff,{min:0, max:1},'neg_decid_diff')
...
Export.image.toDrive({
  ...
  maxPixels:1e13,
  region:export_geom,
  folder:'forcing_export_2_21_2021'
});
...
Map.addLayer(sum_diff,{min:-.05, max:.05, palette:'1178C8,FFC560,B62809'},'sum_alb_diff',false);
Map.addLayer(spr_diff,{min:-.5, max:.5, palette:'1178C8,FFC560,B62809'},'spr_alb_diff',false);
Map.addLayer(fall_diff,{min:-.25, max:.25, palette:'1178C8,FFC560,B62809'},'fall_alb_diff',false);

Map.addLayer(sum_forc,{min:-.05, max:.05, palette:'1178C8,FFC560,B62809'},'sum_forc',false);
Map.addLayer(spr_forc,{min:-.5, max:.5, palette:'1178C8,FFC560,B62809'},'spr_forc',false);
Map.addLayer(fall_forc,{min:-.25, max:.25, palette:'1178C8,FFC560,B62809'},'fall_forc',false);
...
// recent fire
var pos_spr_area_val_rec_area = ee.Number(pos_spr_area_image.multiply(recent_fire_mask).reduceRegion({reducer:ee.Reducer.sum(),
                                                              geometry:boreal_geom,
                                                              scale:calc_scale,
                                                              maxPixels:1e13}).get('area'));
var neg_spr_area_val_rec_area = ee.Number(neg_spr_area_image.multiply(recent_fire_mask).reduceRegion({reducer:ee.Reducer.sum(),
                                                              geometry:boreal_geom,
                                                              scale:calc_scale,
                                                              maxPixels:1e13}).get('area'));

var pos_sum_area_val_rec_area = ee.Number(pos_sum_area_image.multiply(recent_fire_mask).reduceRegion({reducer:ee.Reducer.sum(),
                                                              geometry:boreal_geom,
                                                              scale:calc_scale,
                                                              maxPixels:1e13}).get('area'));    
var neg_sum_area_val_rec_area = ee.Number(neg_sum_area_image.multiply(recent_fire_mask).reduceRegion({reducer:ee.Reducer.sum(),
                                                              geometry:boreal_geom,
                                                              scale:calc_scale,
                                                              maxPixels:1e13}).get('area')) ;       

var pos_fall_area_val_rec_area = ee.Number(pos_fall_area_image.multiply(recent_fire_mask).reduceRegion({reducer:ee.Reducer.sum(),
                                                              geometry:boreal_geom,
                                                              scale:calc_scale,
                                                              maxPixels:1e13}).get('area'));   
var neg_fall_area_val_rec_area = ee.Number(neg_fall_area_image.multiply(recent_fire_mask).reduceRegion({reducer:ee.Reducer.sum(),
                                                              geometry:boreal_geom,
                                                              scale:calc_scale,
                                                              maxPixels:1e13}).get('area'));    
...
// interm fire
var pos_spr_area_val_intr_area = ee.Number(pos_spr_area_image.multiply(spr_forc).multiply(interm_fire_mask).reduceRegion({reducer:ee.Reducer.sum(),
                                                              geometry:boreal_geom,
                                                              scale:calc_scale,
                                                              maxPixels:1e13}).get('area')) ;   
var neg_spr_area_val_intr_area = ee.Number(neg_spr_area_image.multiply(interm_fire_mask).reduceRegion({reducer:ee.Reducer.sum(),
                                                              geometry:boreal_geom,
                                                              scale:calc_scale,
                                                              maxPixels:1e13}).get('area'));    

var pos_sum_area_val_intr_area = ee.Number(pos_sum_area_image.multiply(interm_fire_mask).reduceRegion({reducer:ee.Reducer.sum(),
                                                              geometry:boreal_geom,
                                                              scale:calc_scale,
                                                              maxPixels:1e13}).get('area')) ;    
var neg_sum_area_val_intr_area = ee.Number(neg_sum_area_image.multiply(interm_fire_mask).reduceRegion({reducer:ee.Reducer.sum(),
                                                              geometry:boreal_geom,
                                                              scale:calc_scale,
                                                              maxPixels:1e13}).get('area'));     

var pos_fall_area_val_intr_area = ee.Number(pos_fall_area_image.multiply(interm_fire_mask).reduceRegion({reducer:ee.Reducer.sum(),
                                                              geometry:boreal_geom,
                                                              scale:calc_scale,
                                                              maxPixels:1e13}).get('area')) ;    
var neg_fall_area_val_intr_area = ee.Number(neg_fall_area_image.multiply(interm_fire_mask).reduceRegion({reducer:ee.Reducer.sum(),
                                                              geometry:boreal_geom,
                                                              scale:calc_scale,
                                                              maxPixels:1e13}).get('area'));     
...
// old fire                                                              
var pos_spr_area_val_old_area = ee.Number(pos_spr_area_image.multiply(old_fire_mask).reduceRegion({reducer:ee.Reducer.sum(),
                                                              geometry:boreal_geom,
                                                              scale:calc_scale,
                                                              maxPixels:1e13}).get('area'));    
var neg_spr_area_val_old_area = ee.Number(neg_spr_area_image.multiply(old_fire_mask).reduceRegion({reducer:ee.Reducer.sum(),
                                                              geometry:boreal_geom,
                                                              scale:calc_scale,
                                                              maxPixels:1e13}).get('area'));   

var pos_sum_area_val_old_area = ee.Number(pos_sum_area_image.multiply(old_fire_mask).reduceRegion({reducer:ee.Reducer.sum(),
                                                              geometry:boreal_geom,
                                                              scale:calc_scale,
                                                              maxPixels:1e13}).get('area'));     
var neg_sum_area_val_old_area = ee.Number(neg_sum_area_image.multiply(old_fire_mask).reduceRegion({reducer:ee.Reducer.sum(),
                                                              geometry:boreal_geom,
                                                              scale:calc_scale,
                                                              maxPixels:1e13}).get('area')) ;    

var pos_fall_area_val_old_area = ee.Number(pos_fall_area_image.multiply(old_fire_mask).reduceRegion({reducer:ee.Reducer.sum(),
                                                              geometry:boreal_geom,
                                                              scale:calc_scale,
                                                              maxPixels:1e13}).get('area')) ;    
var neg_fall_area_val_old_area = ee.Number(neg_fall_area_image.multiply(old_fire_mask).reduceRegion({reducer:ee.Reducer.sum(),
                                                              geometry:boreal_geom,
                                                              scale:calc_scale,                      
                                                              maxPixels:1e13}).get('area'));    
                                                              
//fire
var pos_spr_area_val_fc_area= ee.Number(pos_spr_area_image.multiply(fire_mask).reduceRegion({reducer:ee.Reducer.sum(),
                                                         geometry:boreal_geom,
                                                         scale:calc_scale,
                                                         maxPixels:1e13}).get('area'));   
var neg_spr_area_val_fc_area= ee.Number(neg_spr_area_image.multiply(fire_mask).reduceRegion({reducer:ee.Reducer.sum(),
                                                         geometry:boreal_geom,
                                                         scale:calc_scale,
                                                         maxPixels:1e13}).get('area'));
                                                         
var pos_sum_area_val_fc_area= ee.Number(pos_sum_area_image.multiply(fire_mask).reduceRegion({reducer:ee.Reducer.sum(),
                                                         geometry:boreal_geom,
                                                         scale:calc_scale,
                                                         maxPixels:1e13}).get('area'));     
var neg_sum_area_val_fc_area= ee.Number(neg_sum_area_image.multiply(fire_mask).reduceRegion({reducer:ee.Reducer.sum(),
                                                         geometry:boreal_geom,
                                                         scale:calc_scale,
                                                         maxPixels:1e13}).get('area'));    

var pos_fall_area_val_fc_area = ee.Number(pos_fall_area_image.multiply(fire_mask).reduceRegion({reducer:ee.Reducer.sum(),
                                                         geometry:boreal_geom,
                                                         scale:calc_scale,
                                                         maxPixels:1e13}).get('area')) ;
                                                         
var neg_fall_area_val_fc_area = ee.Number(neg_fall_area_image.multiply(fire_mask).reduceRegion({reducer:ee.Reducer.sum(),
                                                         geometry:boreal_geom,
                                                         scale:calc_scale,                      
                                                         maxPixels:1e13}).get('area')) ;        
                                                              
//all
var pos_spr_area_val_all_area= ee.Number(pos_spr_area_image.multiply(tc_mask).reduceRegion({reducer:ee.Reducer.sum(),
                                                         geometry:boreal_geom,
                                                         scale:calc_scale,
                                                         maxPixels:1e13}).get('area'));
var neg_spr_area_val_all_area= ee.Number(neg_spr_area_image.multiply(tc_mask).reduceRegion({reducer:ee.Reducer.sum(),
                                                         geometry:boreal_geom,
                                                         scale:calc_scale,
                                                         maxPixels:1e13}).get('area'));   

var pos_sum_area_val_all_area= ee.Number(pos_sum_area_image.multiply(tc_mask).reduceRegion({reducer:ee.Reducer.sum(),
                                                         geometry:boreal_geom,
                                                         scale:calc_scale,
                                                         maxPixels:1e13}).get('area'));   
var neg_sum_area_val_all_area= ee.Number(neg_sum_area_image.multiply(tc_mask).reduceRegion({reducer:ee.Reducer.sum(),
                                                         geometry:boreal_geom,
                                                         scale:calc_scale,
                                                         maxPixels:1e13}).get('area'));    

var pos_fall_area_val_all_area = ee.Number(pos_fall_area_image.multiply(tc_mask).reduceRegion({reducer:ee.Reducer.sum(),
                                                         geometry:boreal_geom,
                                                         scale:calc_scale,
                                                         maxPixels:1e13}).get('area'));    
var neg_fall_area_val_all_area = ee.Number(neg_fall_area_image.multiply(tc_mask).reduceRegion({reducer:ee.Reducer.sum(),
                                                              geometry:boreal_geom,
                                                              scale:calc_scale,                      
                                                              maxPixels:1e13}).get('area'));   
...
print('pos_spr_area_val_rec_area:',   pos_spr_area_val_rec_area);
print('neg_spr_area_val_rec_area:',   neg_spr_area_val_rec_area);
print('pos_sum_area_val_rec_area:',   pos_sum_area_val_rec_area);
print('neg_sum_area_val_rec_area:',   neg_sum_area_val_rec_area);
print('pos_fall_area_val_rec_area:',  pos_fall_area_val_rec_area);
print('neg_fall_area_val_rec_area:',  neg_fall_area_val_rec_area)                                                    

print('pos_spr_area_val_intr_area:',  pos_spr_area_val_intr_area);
print('neg_spr_area_val_intr_area:',  neg_spr_area_val_intr_area);
print('pos_sum_area_val_intr_area:',  pos_sum_area_val_intr_area);
print('neg_sum_area_val_intr_area:',  neg_sum_area_val_intr_area);
print('pos_fall_area_val_intr_area:', pos_fall_area_val_intr_area);
print('neg_fall_area_val_intr_area:', neg_fall_area_val_intr_area)      

print('pos_spr_area_val_old_area:',   pos_spr_area_val_old_area);
print('neg_spr_area_val_old_area:',   neg_spr_area_val_old_area);
print('pos_sum_area_val_old_area:',   pos_sum_area_val_old_area);
print('neg_sum_area_val_old_area:',   neg_sum_area_val_old_area);
print('pos_fall_area_val_old_area:',  pos_fall_area_val_old_area);  
print('neg_fall_area_val_old_area:',  neg_fall_area_val_old_area);

print('pos_spr_area_val_fc_area:',    pos_spr_area_val_fc_area);
print('neg_spr_area_val_fc_area:',    neg_spr_area_val_fc_area);
print('pos_sum_area_val_fc_area:',    pos_sum_area_val_fc_area);
print('neg_sum_area_val_fc_area:',    neg_sum_area_val_fc_area);
print('pos_fall_area_val_fc_area:',   pos_fall_area_val_fc_area);
print('neg_fall_area_val_fc_area:',   neg_fall_area_val_fc_area);


print('pos_spr_area_val_all_area:',   pos_spr_area_val_all_area);
print('neg_spr_area_val_all_area:',   neg_spr_area_val_all_area);
print('pos_sum_area_val_all_area:',   pos_sum_area_val_all_area);
print('neg_sum_area_val_all_area:',   neg_sum_area_val_all_area);
print('pos_fall_area_val_all_area:',  pos_fall_area_val_all_area);
print('neg_fall_area_val_all_area:',  neg_fall_area_val_all_area)     
...
print('spr_area_val_fc:', ee.Number(neg_spr_area_val_fc).add(pos_spr_area_val_fc))
print('pos_sum_area_val_fc:',  pos_sum_area_val_fc);
print('neg_sum_area_val_fc:',  neg_sum_area_val_fc);
print('sum_area_val_fc:', ee.Number(neg_sum_area_val_fc).add(pos_sum_area_val_fc))
print('pos_fall_area_val_fc:',  pos_fall_area_val_fc);
print('neg_fall_area_val_fc:',  neg_fall_area_val_fc);
print('fall_area_val_fc:', ee.Number(neg_fall_area_val_fc).add(pos_fall_area_val_fc))
...
print('spr_area_u_val_fc:', ee.Number(neg_spr_area_u_val_fc).add(pos_spr_area_u_val_fc))

print('pos_sum_area_u_val_fc:',  pos_sum_area_u_val_fc);
print('neg_sum_area_u_val_fc:',  neg_sum_area_u_val_fc);
print('sum_area_u_val_fc:', ee.Number(neg_sum_area_u_val_fc).add(pos_sum_area_u_val_fc))

print('pos_fall_area_u_val_fc:',  pos_fall_area_u_val_fc);
print('neg_fall_area_u_val_fc:',  neg_fall_area_u_val_fc)      
print('fall_area_u_val_fc:', ee.Number(neg_fall_area_u_val_fc).add(pos_fall_area_u_val_fc))
