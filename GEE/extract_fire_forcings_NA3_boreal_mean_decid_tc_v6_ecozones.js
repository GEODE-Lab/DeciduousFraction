...
alb_sum2000 = alb_sum2000.toFloat().multiply(0.001)
alb_spr2000 = alb_spr2000.toFloat().multiply(0.001)
alb_fal2000 = alb_fal2000.toFloat().multiply(0.001)

alb_sum2015 = alb_sum2015.toFloat().multiply(0.001)
alb_spr2015 = alb_spr2015.toFloat().multiply(0.001)
alb_fal2015 = alb_fal2015.toFloat().multiply(0.001)
...
var tc_diff92 = tc2015.subtract(tc1992).updateMask(tc_mask).multiply(land).multiply(0.01);

var tc_udiff = ee.Image().expression('((a**2 + b**2) ** 0.5)',{'a': tc2000u.toFloat(), 'b': tc2015u.toFloat()}).updateMask(tc_mask).multiply(land).multiply(0.01);
//var tc_udiff = (ee.Image(tc2000u.toFloat()).add(ee.Image(tc2015u).toFloat())).updateMask(tc_mask).multiply(land).multiply(0.01);

Map.addLayer(tc_diff,vis_tc_diff,'tc_diff',false);
Map.addLayer(tc_udiff,vis_tc_diff,'tc_udiff',false);

var decid_diff = ee.Image(decid2015).subtract(ee.Image(decid2000)).multiply(land).multiply(canada_ak).toFloat().multiply(0.01);
var decid_diff92 = ee.Image(decid2015).subtract(ee.Image(decid1992)).updateMask(tc_mask).multiply(land).multiply(canada_ak).toFloat().multiply(0.01);
...
var decid_cdiff = ee.Image(decid2015.multiply(tc2015)).subtract(ee.Image(decid2000.multiply(tc2000))).updateMask(tc_mask).multiply(land).multiply(canada_ak).toFloat().multiply(0.0001);
var decid_cdiff92 = ee.Image(decid2015.multiply(tc2015)).subtract(ee.Image(decid1992.multiply(tc1992))).updateMask(tc_mask).multiply(land).multiply(canada_ak).toFloat().multiply(0.0001);

var evergrn_cdiff = tc_diff.subtract(decid_cdiff);


//var decid_udiff = (ee.Image(decid2000u.toFloat()).add(ee.Image(decid2015u).toFloat())).updateMask(tc_mask).multiply(land).multiply(0.01);
//throw('stop')

Map.addLayer(decid_diff,vis_decid_diff,'decid_diff', false);
Map.addLayer(decid_cdiff,vis_decid_diff,'decid_cdiff', true);
Map.addLayer(evergrn_cdiff,vis_decid_diff,'evergrn_cdiff', true);
...
var pos_decid_cdiff = decid_cdiff.updateMask(decid_cdiff.gt(0)).multiply(area_img);
var neg_decid_cdiff = decid_cdiff.updateMask(decid_cdiff.lte(0)).multiply(area_img);

var pos_evergrn_cdiff = evergrn_cdiff.updateMask(evergrn_cdiff.gt(0)).multiply(area_img);
var neg_evergrn_cdiff = evergrn_cdiff.updateMask(evergrn_cdiff.lte(0)).multiply(area_img);




// mean increase in decid frac (rel abundance)
var decid_diff_pos92 = decid_diff92.updateMask(decid_diff92.gt(ulim)).unmask(0.0).multiply(area_img)
//var decid_udiff_pos = decid_udiff.updateMask(decid_diff.gt(ulim)).unmask(0.0).multiply(area_img)
var decid_diff_pos_area92 = area_img.updateMask(decid_diff92.gt(ulim)).unmask(0.0)
var decid_cdiff_pos_area92 = area_img.updateMask(decid_cdiff92.gt(0.0)).unmask(0.0)

var decid_diff_neg92 = decid_diff92.updateMask(decid_diff92.lte(llim)).unmask(0.0).multiply(area_img)
//var decid_udiff_neg = decid_udiff.updateMask(decid_diff.lte(llim)).unmask(0.0).multiply(area_img)
var decid_diff_neg_area92 = area_img.updateMask(decid_diff92.lte(llim)).unmask(0.0)
var decid_cdiff_neg_area92 = area_img.updateMask(decid_cdiff92.lte(0.0)).unmask(0.0)

// mean increase in tree canopy
var tc_diff_pos92 = tc_diff92.updateMask(tc_diff92.gt(0.0)).unmask(0.0).multiply(area_img)
//var tc_udiff_pos = tc_udiff.updateMask(tc_diff.gt(0.0)).unmask(0.0).multiply(area_img)
var tc_diff_pos_area92 = area_img.updateMask(tc_diff92.gt(0.0)).unmask(0.0)

// mean decrease in tree canopy
var tc_diff_neg92 = tc_diff92.updateMask(tc_diff92.lte(0.0)).unmask(0.0).multiply(area_img)
//var tc_udiff_neg = tc_udiff.updateMask(tc_diff.lte(0.0)).unmask(0.0).multiply(area_img)
var tc_diff_neg_area92 = area_img.updateMask(tc_diff92.lte(0.0)).unmask(0.0)
...
var spr_pforc = spr_forc.updateMask(spr_pos_mask).multiply(area_img)
var spr_nforc = spr_forc.updateMask(spr_neg_mask).multiply(area_img)

Map.addLayer(spr_forc.updateMask(spr_pos_mask),{min:-0.25, max:1, palette:'B62809,FFC560,1178C8'},'spr_pforc');

Map.addLayer(spr_forc.updateMask(spr_neg_mask),{min:-1, max:0, palette:'B62809,FFC560,1178C8'},'spr_nforc');



Map.addLayer(tc_mask_min, {min:0, max:1},'tc_mask')
...
var avg_forc = ee.ImageCollection([sum_forc, spr_forc, fall_forc]).reduce(ee.Reducer.mean())
var avg_energy_total_img = avg_forc.multiply(area_img)
var avg_energy_fire_img = avg_energy_total_img.updateMask(fire_mask)

var area_img_fire = fire_mask.multiply(area_img)

Map.addLayer(avg_energy_total_img, {}, 'avg_energy_total_img')
Map.addLayer(avg_energy_fire_img, {}, 'avg_energy_fire_img')

var total_area = area_img.reduceRegion({reducer: ee.Reducer.sum(), geometry: boreal_geom, scale: 250, maxPixels: 1e13})
var fire_area = area_img_fire.reduceRegion({reducer: ee.Reducer.sum(), geometry: boreal_geom, scale: 250, maxPixels: 1e13})

var fire_energy = avg_energy_fire_img.reduceRegion({reducer: ee.Reducer.sum(), geometry: boreal_geom, scale: 250, maxPixels: 1e13})
var total_energy = avg_energy_total_img.reduceRegion({reducer: ee.Reducer.sum(), geometry: boreal_geom, scale: 250, maxPixels: 1e13})

print('total area:', total_area)
print('fire area:', fire_area)

print('total energy', total_energy)
print('fire energy', fire_energy)



var data_img = (              decid_diff_pos
                              .addBands(decid_udiff_pos)
                              ...
                              .addBands(decid_diff_pos92)
                              .addBands(decid_diff_pos_area92)
                              .addBands(decid_cdiff_pos_area92)
                              .addBands(decid_diff_neg92)
                              .addBands(decid_diff_neg_area92)
                              .addBands(decid_cdiff_neg_area92)
                              ...
                              .addBands(tc_diff_pos92)
                              .addBands(tc_diff_pos_area92)
                              .addBands(tc_diff_neg92)
                              .addBands(tc_diff_neg_area92))
                              ...
                              .rename(['decid_diff_pos',
                                       ...
                                       'decid_diff_pos92',
                                       'decid_diff_pos_area92',
                                       'decid_cdiff_pos_area92',
                                       'decid_diff_neg92',
                                       'decid_diff_neg_area92',
                                       'decid_cdiff_neg_area92',
...
Export.table.toDrive({
  ...
  folder:'ecozones_area_summary_v5'
