...
var area_img = ee.Image.pixelArea().multiply(tc_mask).multiply(land).rename('area');

Map.addLayer(area_img)

// values ---------------------------------------------------------------------------------------------------------------------

// spr
var pos_spr_watts = area_img.updateMask(spr_forc.gt(0)).multiply(spr_forc).rename('pos_spr_watts');
var neg_spr_watts = area_img.updateMask(spr_forc.lt(0)).multiply(spr_forc).rename('neg_spr_watts');

var pos_spr_area = area_img.updateMask(spr_forc.gt(0)).rename('pos_spr_area');
var neg_spr_area = area_img.updateMask(spr_forc.lt(0)).rename('neg_spr_area');
//Map.addLayer(pos_spr_watts,{min:0,max:100},'pos_spr_watts')

// sum
var pos_sum_watts = area_img.updateMask(sum_forc.gt(0)).multiply(sum_forc).rename('pos_sum_watts');
var neg_sum_watts = area_img.updateMask(sum_forc.lt(0)).multiply(sum_forc).rename('neg_sum_watts');

var pos_sum_area = area_img.updateMask(sum_forc.gt(0)).rename('pos_sum_area');
var neg_sum_area = area_img.updateMask(sum_forc.lt(0)).rename('neg_sum_area');

// fall
var pos_fal_watts = area_img.updateMask(fall_forc.gt(0)).multiply(fall_forc).rename('pos_fal_watts');
var neg_fal_watts = area_img.updateMask(fall_forc.lt(0)).multiply(fall_forc).rename('neg_fal_watts');

var pos_fal_area = area_img.updateMask(fall_forc.gt(0)).rename('pos_fal_area');
var neg_fal_area = area_img.updateMask(fall_forc.lt(0)).rename('neg_fal_area');


var comp_image = pos_spr_watts.addBands(neg_spr_watts)
                              .addBands(pos_sum_watts)
                              .addBands(neg_sum_watts)
                              .addBands(pos_fal_watts)
                              .addBands(neg_fal_watts)
                              .addBands(pos_spr_area)
                              .addBands(neg_spr_area)
                              .addBands(pos_sum_area)
                              .addBands(neg_sum_area)
                              .addBands(pos_fal_area)
                              .addBands(neg_fal_area)



var data_coll_all = comp_image.reduceRegions(ecozones, ee.Reducer.sum(), calc_scale).map(function(elem){return elem.setGeometry(null)})


var data_coll_fire = comp_image.updateMask(top_fire_layer.gt(0)).reduceRegions(ecozones, ee.Reducer.sum(), calc_scale).map(function(elem){return elem.setGeometry(null)})
...
Export.table.toDrive({
  ...
  folder:'ecozones_forcing_area_summary_v' + version
