...
//Map.addLayer(recent_fire_mask,{min:1998, max:2018},'recent_fire_mask')


var data_img = (decid2000.multiply(tc2000).multiply(area_img)
                              .addBands((ee.Image(100.0).subtract(decid2000)).multiply(tc2000).multiply(area_img))
                              .addBands(decid2015.multiply(tc2015).multiply(area_img))
                              .addBands((ee.Image(100.0).subtract(decid2015)).multiply(tc2015).multiply(area_img))
                              .addBands(decid2000.multiply(area_img))
                              .addBands(decid2000u.multiply(area_img))
                              .addBands(decid2015.multiply(area_img))
                              .addBands(decid2015u.multiply(area_img))
                              .addBands(tc2000.multiply(area_img))
                              .addBands(tc2000u.multiply(area_img))
                              .addBands(tc2015.multiply(area_img))
                              .addBands(tc2015u.multiply(area_img))
                              .addBands(recent_fire_mask.multiply(area_img))
                              .addBands(interm_fire_mask.multiply(area_img))
                              .addBands(old_fire_mask.multiply(area_img))
                              .addBands(area_img))
                              .rename(['decidcover2000',
                                       'evergreencover2000',
                                       'decidcover2015',
                                       'evergreencover2015',
                                       'decid2000',
                                       ...
                                       'old_fire_mask',
                                       'area'
                                        ]).multiply(land).multiply(tc_mask).multiply(boreal_mask)
...
Export.table.toDrive({
  ...
  fileNamePrefix:'ecozones_area_summary_all_decid_tc_only_2000_2015_v' + version+ '_'+calc_scale,
  description:'ecozones_area_summary_all_decid_tc_only_2000_2015_v' + version+ '_'+calc_scale,
