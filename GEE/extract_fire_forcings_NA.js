...
var land_extent = ee.Image("users/masseyr44/decid/land_extent_NA"),
    ...
    albedo_kernel = ee.Image("users/masseyr44/albedo_products/monkernel1_albedo_sw_clr_kernel_reproj_NA_500m"),
...
var tc = ee.ImageCollection([tc2015,tc2010,tc2005,tc2000,tc1992]).reduce(ee.Reducer.max())
//Map.addLayer(boreal2)
Map.addLayer(tc.updateMask(tc.gt(25).multiply(land_extent)), {min:0, max:100, palette:'FF3600,0051FF'},'tc_max',false)
...
decid2015e = decid2015e.rename('band_1').addBands(ee.Image(1).rename('quality').byte())
decid2000e = decid2000e.rename('band_1').addBands(ee.Image(1).rename('quality').byte())

print(decid2000e)
Map.addLayer(decid2000e,{},'decid2000e',false)

decid2015 = decid2015.rename('band_1').addBands(ee.Image(0).rename('quality').byte())
decid2000 = decid2000.rename('band_1').addBands(ee.Image(0).rename('quality').byte())
...
Map.addLayer(final.multiply(land_extent),{min:0, max:1, palette:'0F5008,F9DC4D'},'final_decid',false)

var fin_alb = final.expression(expr_tc25, {'x': final})

var change_decid = final.subtract(initial).multiply(land_extent).updateMask(tc.gt(25)).clip(boreal3)
...
Map.addLayer(albedo_kernel, {min:-5 ,max:0, palette:'FF3600,0051FF'},'albedo_kernel',false)

var forcing = fin_alb.subtract(ini_alb).updateMask(tc.gt(25)).multiply(land_extent).multiply(albedo_kernel).rename('forcing')
...
var fire_filtered = can_fire2.merge(ak_fire2).filterMetadata('SIZE_HA','greater_than', 500).filterMetadata('YEAR','not_less_than',1998).filterMetadata('YEAR','not_greater_than',2018).filterMetadata('SIZE_HA','greater_than', 500)  // > 500 hectares
...
print(fire_1998_2018.size())
print(fire_1998_2018_.size())
Map.addLayer(fire_1998_2018_, {color: 'green'}, 'fire_1998_2018')


var image_stack = ee.Image(forcing_tc25.addBands(change_decid).addBands(tc_max))
                          .multiply(ee.Image.pixelArea()).addBands(ee.Image.pixelArea())
                          .select([0,1,2,3],['forc_tc25','decid_ch_00_15','tc_max','area'])

var fire_1998_2018_extract = image_stack.reduceRegions(fire_1998_2018_, ee.Reducer.sum(), 30).map(function(feat){ return feat.setGeometry(feat.geometry().centroid())})
var fire_1978_1998_extract = image_stack.reduceRegions(fire_1978_1998, ee.Reducer.sum(), 30).map(function(feat){ return feat.setGeometry(feat.geometry().centroid())})
var fire_1950_1978_extract = image_stack.reduceRegions(fire_1950_1978, ee.Reducer.sum(), 30).map(function(feat){ return feat.setGeometry(feat.geometry().centroid())})

var all_fire_extract_NA = image_stack.reduceRegions(fire_filtered, ee.Reducer.sum(), 30).map(function(feat){ return feat.setGeometry(feat.geometry().centroid())}) 

Export.table.toDrive({
  collection: fire_1998_2018_extract,
  description: 'fire_1998_2018_extract',
  folder: 'forcing_extract_gee',
  fileNamePrefix: 'fire_1998_2018_extract'
})


Export.table.toDrive({
  collection: fire_1978_1998_extract,
  description: 'fire_1978_1998_extract',
  folder: 'forcing_extract_gee',
  fileNamePrefix: 'fire_1978_1998_extract'
})

Export.table.toDrive({
  collection: fire_1950_1978_extract,
  description: 'fire_1950_1978_extract',
  folder: 'forcing_extract_gee',
  fileNamePrefix: 'fire_1950_1978_extract'
})

Export.table.toDrive({
  collection: all_fire_extract_NA,
  description: 'all_fire_extract_NA',
  folder: 'forcing_extract_gee',
  fileNamePrefix: 'all_fire_extract_NA',
