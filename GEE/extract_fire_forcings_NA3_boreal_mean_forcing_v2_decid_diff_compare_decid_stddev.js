...
Map.addLayer(canada_boreal,{},'canada_boreal')

Map.addLayer(boreal_mask,{},'boreal_mask')

Map.addLayer(boreal_geom_2,{},'boreal_geom_2')

var decid_1992_can_std = decid1992.multiply(boreal_mask).clip(canada_boreal).reduceRegion({reducer:ee.Reducer.stdDev(),
                                                              geometry:boreal_geom,
                                                              scale:calc_scale,
                                                              maxPixels:1e13}) ;  

var decid_2015_can_std = decid2015.multiply(boreal_mask).clip(canada_boreal).reduceRegion({reducer:ee.Reducer.stdDev(),
                                                              geometry:boreal_geom,
                                                              scale:calc_scale,
                                                              maxPixels:1e13}) ;    
                                                              
var decid_2015_std = decid2015.multiply(boreal_mask).reduceRegion({reducer:ee.Reducer.stdDev(),
                                                              geometry:boreal_geom,
                                                              scale:calc_scale,
                                                              maxPixels:1e13}) ;         
                                                              
var decid_2000_std = decid2000.multiply(boreal_mask).reduceRegion({reducer:ee.Reducer.stdDev(),
                                                              geometry:boreal_geom,
                                                              scale:calc_scale,
                                                              maxPixels:1e13}) ;       
                                                              
                                                              
print('decid_1992_can_std', decid_1992_can_std)
print('decid_2015_can_std', decid_2015_can_std)
print('decid_2015_std', decid_2015_std)
print('decid_2000_std', decid_2000_std)
...
// values ---------------------------------------------------------------------------------------------------------------------
var ecozone_stats = decid_diff.reduceRegions(ee.FeatureCollection(ecozones), ee.Reducer.sum(), calc_scale)

print(ecozone_stats)


// decid
var decid_area_image_0_lim = area_img.updateMask(decid_diff.gt(0.0));
var decid_area_image__lim_0 = area_img.updateMask(decid_diff.lte(0.0));

var ecozone_stats_image_0_lim = decid_area_image_0_lim.reduceRegions(ee.FeatureCollection(ecozones), ee.Reducer.sum(), calc_scale)
var ecozone_stats_image__lim_0 = decid_area_image__lim_0.reduceRegions(ee.FeatureCollection(ecozones), ee.Reducer.sum(), calc_scale)



var decid_area_image_0_lim_sig = area_img.updateMask(decid_diff.gt(0.0)).multiply(sig_decid);
var decid_area_image__lim_0_sig = area_img.updateMask(decid_diff.lte(0.0)).multiply(sig_decid);

var ecozone_stats_image_0_lim_sig = decid_area_image_0_lim_sig.reduceRegions(ee.FeatureCollection(ecozones), ee.Reducer.sum(), calc_scale)
var ecozone_stats_image__lim_0_sig = decid_area_image__lim_0_sig.reduceRegions(ee.FeatureCollection(ecozones), ee.Reducer.sum(), calc_scale)




print('ecozone_stats_image_0_lim', ecozone_stats_image_0_lim)
print('ecozone_stats_image__lim_0', ecozone_stats_image__lim_0)

print('ecozone_stats_image_0_lim_sig', ecozone_stats_image_0_lim_sig)
print('ecozone_stats_image__lim_0_sig', ecozone_stats_image__lim_0_sig)
