...
def md_clean(sample_dicts,
    ...
    if len(sample_dicts) > len(bandnames):
        ...
        loc = list(ii for ii, x in enumerate(md_vec) if
                   (x <= np.percentile(md_vec, md_cutoff) and x != np.nan))
...
if __name__ == '__main__':
    ...
    outfile = folder + "gee_data_cleaning_v{}_median.csv".format(str(version))
    ...
    # initialize empty lists for each year bin
    ...
    # bin each sample in the year bins from above
    for site_id_year, site_dict in composite_data_dict.items():
        for i, years in enumerate(year_bins):
            if years[0] <= site_dict['site_year'] <= years[1]:
                year_samp[i].append(site_dict)

    samp_lengths = list(len(elem) for elem in year_samp)
    print('Before combining samples in same time period: {}'.format(str(samp_lengths)))

    # if multiple samples in a year bin, calculate median of those samples
    for i, samp_list in enumerate(year_samp):

        # get site IDs for each site
        site_ids = list(set(list(site_dict['site'] for site_dict in samp_list)))

        for site_id in site_ids:

            # list of same site IDs with different attributes
            same_site_samp_list = list(site_dict for site_dict in samp_list if site_dict['site'] == site_id)

            # calculate median of deciduous fraction
            decid_frac = percentile(list(site_dict['decid_frac'] for site_dict in same_site_samp_list), 50)
            # decid_frac = mean(list(site_dict['decid_frac'] for site_dict in same_site_samp_list))

            index = list(j for j, site_dict in enumerate(same_site_samp_list)
                         if site_dict['decid_frac'] == decid_frac)[0]

            # add the median year and deciduous fraction to a list
            year_samp_reduced[i].append(same_site_samp_list[index])

    samp_lengths = list(len(elem) for elem in year_samp_reduced)
    print('After combining samples in same time period: {}'.format(str(samp_lengths)))

    # append all the samples to one list
    all_samp = list()
    for sublist in year_samp_reduced:
        for elem in sublist:
            all_samp.append(elem)

    print('Total samples before cleaning: {}'.format(str(len(all_samp))))

    # list of deciduous fraction values
    decid_frac_list = list(samp['decid_frac'] for samp in all_samp)
    ...
    out_decid = list(site_samp['decid_frac'] for site_samp in all_cut_samp)
    ...
    perc_out = 100.0*(float(len(decid_frac_list)-len(out_decid))/float(len(decid_frac_list)))
